...
const toggleBlock = async (patient) => {
  ...
  const originalBlacklistedState = patient.user.blacklisted;
  const originalActiveState = patient.user.active;
  
  // Toggle the state immediately in the UI
  patient.user.blacklisted = !patient.user.blacklisted;
  patient.user.active = !patient.user.active;
  ...
  try {
    ...
    patient.user.blacklisted = originalBlacklistedState;
    patient.user.active = originalActiveState;
...
const deletePatient = async (id) => {
  ...
  // Optimistically remove patient from the list immediately
  const patientIndex = patients.value.findIndex(p => p.id === id);
  const removedPatient = patients.value[patientIndex];
  ...
  try {
    ...
    // Revert the optimistic update if the API call fails
    if (removedPatient && patientIndex !== -1) {
      patients.value.splice(patientIndex, 0, removedPatient);
    }

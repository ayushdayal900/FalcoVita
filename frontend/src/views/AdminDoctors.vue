...
const toggleBlock = async (doctor) => {
  ...
  const originalBlacklistedState = doctor.user.blacklisted;
  const originalActiveState = doctor.user.active;
  
  // Toggle the state immediately in the UI
  doctor.user.blacklisted = !doctor.user.blacklisted;
  doctor.user.active = !doctor.user.active;
  
  try {
    // Make the API call in the background
    if (originalBlacklistedState) {
    ...
    // Re-fetch to ensure consistency with backend (especially cache invalidation)
    fetchDoctors();
  } catch (err) {
    // Revert the optimistic update if the API call fails
    doctor.user.blacklisted = originalBlacklistedState;
    doctor.user.active = originalActiveState;
...
const deleteDoctor = async (id) => {
  ...
  // Optimistically remove doctor from the list immediately
  const doctorIndex = doctors.value.findIndex(d => d.id === id);
  const removedDoctor = doctors.value[doctorIndex];
  
  if (doctorIndex !== -1) {
    doctors.value.splice(doctorIndex, 1);
  }
  ...
  try {
    ...
    // Revert the optimistic update if the API call fails
    if (removedDoctor && doctorIndex !== -1) {
      doctors.value.splice(doctorIndex, 0, removedDoctor);
    }

...
const fetchDoctorStats = async () => {
  try {
    ...
    console.log('Dashboard: Current Doctor ID:', doctorId, typeof doctorId);
    ...
    console.log('Dashboard: All Appointments:', allAppointments.length, allAppointments[0]);
    ...
    console.log('Dashboard: Doctor Appointments:', appointments.length);

    const today = new Date().toDateString();
    console.log('Dashboard: Today string:', today);

    todayAppointments.value = appointments.filter(a => {
      const apptDate = new Date(a.appointment_date).toDateString();
      // console.log('Appt Date:', apptDate, 'Status:', a.status);
    ...
    console.log('Dashboard: All Patients:', patients.length, patients[0]);

    // Fix: Patient object is flat, doctor_id is at top level
    patientsList.value = patients.filter(
      p => p.doctor_id == doctorId
    );
    console.log('Dashboard: Doctor Patients:', patientsList.value.length);

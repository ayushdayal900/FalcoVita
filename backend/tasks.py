...
@celery_app.task(name='backend.tasks.export_patient_treatments_async')
def export_patient_treatments_async(patient_id, email):
    """Async task to export patient treatments and send via email"""
    with app.app_context():
        from backend.resources.export_resource import ExportPatientHistoryResource
        import tempfile
        
        try:
            # This is a simplified version - in production, you'd generate the CSV
            ...
            <body style="font-family: Arial, sans-serif; padding: 20px;">
                ...
                <p>Your treatment history export has been completed.</p>
                <p>Please log in to the portal to download your records.</p>
                <p>Best regards,<br>FalcoVita Hospital Team</p>
            </body>
            </html>
            """
            
            send_email(email, 'Treatment Export Completed', html_content)
            return f"Export notification sent to {email}"
        except Exception as e:
